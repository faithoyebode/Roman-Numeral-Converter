function romanNumerals(e){
    ...
    if((typeof num != "number") && (Math.round(num) != num)){
        $("div#output").collapse('show');
        return output.textContent = "Your input is not a valid integer";
    }
    else if (num == 0){
        $("div#output").collapse('show');
        return output.textContent = "Your input must be an integer greater than zero";
    }
    else if(num >= 1000){
        let x1;
    ...
    $("div#output").collapse('show');
    return output.textContent = array.join('');
}
let sendButton = document.querySelector("input#send");
sendButton.addEventListener("click", romanNumerals);
$(document).ready(function(){
    $("#close").click(function(){
        $("div#output").collapse('hide');
    });
